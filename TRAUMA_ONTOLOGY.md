...
│   ├── Not recorded (NR entry)
│   ├── No trauma reported (NA/ empty)
│   └── Any adversity identified (1 or more trauma related entry)
│
├── Timing
│   ├── Childhood
│   ├── Adulthood
...
Not recorded, no trauma reported, uncertainty, CTQ reference, and any identified adversity

Timing

Infancy, childhood, adolescence, adulthood, multiple periods, and unknown timing

Trauma type

Sexual, physical, and emotional abuse; neglect; bullying; conjugal violence; accidents; bereavement; war; and workplace trauma

Relationship context

Father, mother, sibling, extended family, partner, and peer or school references

Family and social context

Parental and family violence, witnessed violence, family instability, parental incapacity, financial adversity, institutional adversity, and neurodevelopment-related adversity

Derived profiles

Developmentally specific trauma, interpersonal trauma, repeated adversity, contextual family adversity, and complex trauma
...
Text is lowercased, accents and punctuation are normalized, and predefined clinical abbreviations are expanded.

Semicolons divide the narrative into event-level semantic blocks.

Trauma type, timing, and relationship indicators are extracted independently within each segment.

Derived combinations are created only when the required concepts occur within the same segment. For example, childhood physical abuse requires both childhood timing and physical abuse within that event block.

A segment may receive multiple labels.

Participant-level rules

Binary segment indicators are aggregated using presence across any segment.

Count variables, such as age mentions and bereavement counts, are summed.

The number of adversity types is calculated from distinct participant-level trauma-type indicators.

Multiple developmental periods are present when events span at least two life periods.

Complex trauma is identified when at least one of the following is present:
...
Empty, NR, NA, ND, or non renseigné

Not recorded

RAS, rien à signaler, pas de trauma, or equivalent

Evaluated with no trauma reported

Substantive event description

Evaluable trauma documentation
